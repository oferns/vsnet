namespace VS.Core.Data.Handlers {
    ...
    public class GetListDecorator<T> : IRequestHandler<GetList<T>, PagedList<T>> where T : class {
        ...
        public GetListDecorator(IRequestHandler<GetList<T>, PagedList<T>> wrappedHandler, IDbClient db, IQuerySyntaxProvider<T> querySyntax, IFilterService<T> filter, ISorterService<T> sorter, IPager pager) {
        ...
        public async Task<PagedList<T>> Handle(GetList<T> request, CancellationToken cancel) {
            ...
            var queryfilter = request.Filter ?? filter.Filter;
